...
object SalesOrderHistoric {
  ...
  def processHistoricData() = {
    ...
    for (i <- 0 to 90) {
      ...
      val prevFull = DataReader.getDataFrameOrNull("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.FULL_MERGE_MODE, date)
      val before7 = TimeUtils.getDateAfterNDays(-7, incrDate)
      val salesRevenue7 = DataReader.getDataFrameOrNull("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.DAILY_MODE, before7)

      val before30 = TimeUtils.getDateAfterNDays(-30, incrDate)
      val salesRevenue30 = DataReader.getDataFrameOrNull("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.DAILY_MODE, before30)

      val before90 = TimeUtils.getDateAfterNDays(-90, incrDate)
      val salesRevenue90 = DataReader.getDataFrameOrNull("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.DAILY_MODE, before90)
      var salesOrderincr: DataFrame = null
      var salesOrderItemincr: DataFrame = null
      if (null == prevFull) {
        salesOrderincr = DataReader.getDataFrameOrNull("/data/input", DataSets.BOB, DataSets.SALES_ORDER, DataSets.FULL_MERGE_MODE, TimeUtils.getDateAfterNDays(-1, TimeConstants.DATE_FORMAT_FOLDER))
        ...
        salesOrderItemincr = DataReader.getDataFrameOrNull("/data/input", DataSets.BOB, DataSets.SALES_ORDER_ITEM, DataSets.FULL_MERGE_MODE, incrDate)
      } else {
        val salesOrder = DataReader.getDataFrameOrNull("/data/input", DataSets.BOB, DataSets.SALES_ORDER, DataSets.DAILY_MODE, incrDate)
        val salesOrderItem = DataReader.getDataFrameOrNull("/data/input", DataSets.BOB, DataSets.SALES_ORDER_ITEM, DataSets.DAILY_MODE, incrDate)
      ...
      var savePath = DataWriter.getWritePath("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.FULL_MERGE_MODE, incrDate)
      var savePathDaily = DataWriter.getWritePath("/data/test/output", DataSets.VARIABLES, DataSets.SALES_ITEM_REVENUE, DataSets.DAILY_MODE, incrDate)
