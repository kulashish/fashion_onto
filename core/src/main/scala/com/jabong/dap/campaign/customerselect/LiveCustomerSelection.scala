...
abstract class LiveCustomerSelection extends CustomerSelection {
  ...
  def customerOrderFilter(skuFilteredData:DataFrame,orderData:DataFrame):DataFrame={
    ...
    val customerFilteredData = skuFilteredData.join(orderData
      ,skuFilteredData.col(ACartVariables.UID).equalTo(orderData.col(CustomerVariables.FK_CUSTOMER)),"left")
...
/*customer sku filter*/
  def customerSkuFilter(customerData:DataFrame):DataFrame={
    ...
    val filteredData = customerData.withColumn(ACartVariables.ACART_SKU1,skuFilter(customerData(ACartVariables.ACART_PRICE_SKU1)
      ,customerData(ACartVariables.ACART_PRICE_SKU1_PRICE)
      ,customerData(ACartVariables.ACART_PRICE_SKU1_TODAY_PRICE))).withColumn(ACartVariables.ACART_SKU2,
        skuFilter(customerData(ACartVariables.ACART_PRICE_SKU2)
          ,customerData(ACartVariables.ACART_PRICE_SKU2_PRICE)
          ,customerData(ACartVariables.ACART_PRICE_SKU1_TODAY_PRICE))).select(ACartVariables.UID,ACartVariables.ACART_SKU1,ACartVariables.ACART_PRICE_SKU1_BRAND,ACartVariables.ACART_PRICE_SKU1_BRICK
        ,ACartVariables.ACART_PRICE_SKU1_CATEGORY,ACartVariables.ACART_PRICE_SKU1_COLOR,
        ACartVariables.ACART_PRICE_SKU1_GENDER,ACartVariables.ACART_SKU2,ACartVariables.ACART_PRICE_SKU2_BRAND
        ,ACartVariables.ACART_PRICE_SKU2_BRICK,ACartVariables.ACART_PRICE_SKU2_CATEGORY,
