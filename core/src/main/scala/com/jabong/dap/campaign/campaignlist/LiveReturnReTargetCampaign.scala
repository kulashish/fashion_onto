...
class LiveReturnReTargetCampaign {
  ...
  def runCampaign(targetCustomersWithOrderItems: DataFrame, yesterdayItrData: DataFrame,
    ...
    CampaignUtils.campaignPostProcess(DataSets.PUSH_CAMPAIGNS, CampaignCommon.RETURN_RETARGET_CAMPAIGN, filteredSkuJoinedItr, false, null, incrDate)

    // ***** email use case
    CampaignUtils.campaignPostProcess(DataSets.EMAIL_CAMPAIGNS, CampaignCommon.RETURN_RETARGET_CAMPAIGN, filteredSkuJoinedItr, false, brickMvpRecommendations, incrDate)
