...
class AcartHourlyCampaign {
  ...
  def runCampaign(acartData: DataFrame, salesOrderData: DataFrame, salesOrderItemData: DataFrame, yesterdayItrData: DataFrame,
    ...
    // ***** email use case
    CampaignUtils.campaignPostProcess(DataSets.EMAIL_CAMPAIGNS, CampaignCommon.ACART_HOURLY_CAMPAIGN, filterSkus, false, brickMvpRecommendations, incrDate)
  }
}
