...
class ShortlistReminderCampaign {
  ...
  def runCampaign(shortlist3rdDayData: DataFrame, brickMvpRecommendations: DataFrame, yesterdayItrData: DataFrame, incrDate: String) = {
    ...
    // ***** email use case
    CampaignUtils.campaignPostProcess(DataSets.EMAIL_CAMPAIGNS, CampaignCommon.SHORTLIST_REMINDER, filteredSku, false, brickMvpRecommendations, incrDate)
  }
