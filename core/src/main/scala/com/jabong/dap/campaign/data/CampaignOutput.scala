...
object CampaignOutput {
  ...
  def saveAcartHourlyFeed(inputData: DataFrame, cmr: DataFrame, acartHourlyFileName: String): DataFrame = {
    ...
    val acartOutData = inputDataNumberSkus
      ...
      .withColumn(CampaignMergedFields.REF_SKU + "1", Udf.getElementInTupleArray(col(CampaignMergedFields.REF_SKUS), lit(0), lit(0)))
      .withColumn(CampaignMergedFields.REF_SKU + "2", Udf.getElementInTupleArray(col(CampaignMergedFields.REF_SKUS), lit(1), lit(0)))
      .withColumn(CampaignMergedFields.REF_SKU + "3", Udf.getElementInTupleArray(col(CampaignMergedFields.REF_SKUS), lit(2), lit(0)))
