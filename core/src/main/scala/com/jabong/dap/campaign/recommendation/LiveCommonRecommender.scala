...
class LiveCommonRecommender extends Recommender with Logging {
   ...
   * @return
   */
  override def generateRecommendation(refSkus: DataFrame, recommendations: DataFrame, recType: String = Recommendation.BRICK_MVP_SUB_TYPE, numRecSkus: Int): DataFrame = {
    ...
    val recommendationGrouped = recommendationSelected.map(row => ((row(0)), (row))).groupByKey().map({ case (key, value) => (key.asInstanceOf[Long], getRecSkus(value, numRecSkus)) })
