...
object MongoFeedGenerator  extends Logging {
  ...
  def   start(params: ParamInfo) {
    ...
    val mongoDbCollection = OptionUtils.getOptValue(params.incrDate, TimeUtils.getDateAfterNDays(-1, TimeConstants.DATE_FORMAT))
    val options = Map("host" -> "172.16.90.53:27017", "credentials" -> "reco,admin,xD8b6jG72", "database" -> "reco", "collection" -> mongoDbCollection)
