...
object DataQualityMethods {
  ...
  def Artemisdaily(hiveContext: HiveContext, day: String, month: String, year: String, tablename: String): String = {
    ...
    /*
    val id = data.filter(data("id") isNull).count()

    message = "Count of null id:" + (id).toString() + "\n"

    data.printSchema().toString
    ...
    val id = ("select count(*) from " + tablename + " where id is NULL and date1=" + day + " and month1=" + month + " and year1=" + year)
