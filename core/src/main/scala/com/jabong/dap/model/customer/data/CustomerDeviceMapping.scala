...
object CustomerDeviceMapping extends Logging {
  ...
  def getLatestDevice(clickStreamInc: DataFrame, cmr: DataFrame, customer: DataFrame): DataFrame = {
    ...
    val joinedDf = clickStream.join(broCust, broCust(CustomerVariables.EMAIL) === clickStream(PageVisitVariables.USER_ID), SQL.OUTER)
      .select(
    ...
    val joined = joinedDf.join(cmr, cmr(CustomerVariables.EMAIL) === joinedDf(CustomerVariables.EMAIL), SQL.OUTER)
      .select(
  ...
  def getDataFrameCsv4mDCF(path: String): DataFrame = {
    ...
    try {
      ...
      val res = df.join(correctRecs, df(CustomerVariables.ID_CUSTOMER) === correctRecs(NEW_ID_CUSTOMER), SQL.LEFT)
        .select(
