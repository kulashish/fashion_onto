...
object PaybackData extends DataFeedsModel {
  ...
  def write(dfWrite: HashMap[String, DataFrame], saveMode: String, incrDate: String) = {
    ...
    DataWriter.writeCsv(dfWrite("paybackIncr").na.fill(""), DataSets.VARIABLES, DataSets.PAYBACK_DATA, DataSets.DAILY_MODE, incrDate, fileDate + "_payback_data", DataSets.IGNORE_SAVEMODE, "true", ";", 1)
  }
}
