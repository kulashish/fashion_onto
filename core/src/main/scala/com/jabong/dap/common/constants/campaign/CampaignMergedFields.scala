package com.jabong.dap.common.constants.campaign

/**
 * Created by rahul for com.jabong.dap.common.constants.campaign on 21/7/15.
 */
object CampaignMergedFields {
  val CAMPAIGN_MAIL_TYPE = "mail_type"
  val END_OF_DATE = "date"
  val DEVICE_ID = "device_id"
  val CUSTOMER_ID = "customer_id"
  val REF_SKU1 = "ref_sku1"
  val REF_SKU2 = "ref_sku2"
  val EMAIL = "email"
  val DOMAIN = "domain"

  val REF_SKUS = "ref_skus"

  val REC_SKUS = "rec_skus"

  val REF_SKU = "ref_sku"

  val deviceId = "deviceId"
  val LIVE_MAIL_TYPE = "LIVE_MAIL_TYPE"
  val LIVE_BRAND = "LIVE_BRAND"
  val LIVE_REF_SKU1 = "LIVE_REF_SKU1"
  val LIVE_BRICK = "LIVE_BRICK"
  val LIVE_PROD_NAME = "LIVE_PROD_NAME"
  val LIVE_CART_URL = "LIVE_CART_URL"



  val RECOMMENDATIONS = "recommendations"

  //Expected names
  val LIVE_REF_SKU = "LIVE_REF_SKU"
  val LIVE_REC_SKU = "LIVE_REC_SKU"
  val TYPO_MOBILE_PERMISION_STATUS = "MOBILE_PERMISION_STATUS"
  val LAST_UPDATED_DATE = "LAST_UPDATED_DATE"
  val COUNTRY_CODE = "COUNTRY_CODE"

  // Calendar Campaign

  val CALENDAR_REF_SKU = "CLDR_REF_SKU"
  val CALENDAR_REC_SKU = "CLDR_REC_SKU"
  val CALENDAR_MAIL_TYPE = "CLDR_MAIL_TYPE"
  val CALENDAR_REF_BRAND = "CLDR_REF_BRAND"
  val CALENDAR_REC_BRAND = "CLDR_REC_BRAND"
  val CALENDAR_REF_BRICK = "CLDR_REF_BRICK"
  val CALENDAR_REC_BRICK = "CLDR_REC_BRICK"
  val CALENDAR_CITY = "CLDR_CITY"
  val CALENDAR_COLOR = "CLDR_COLOR"
  val CALENDAR_PRICE_POINT = "CLDR_PRICE_POINT"

  // replenish campaign
  val PURCHASED_DATE = "PURCHASED_DATE"
  val REC_SKU = "REC_SKU"
  val LIVE_REF = "LIVE_REF_1"
  val PRODUCT_GENDER = "PRODUCT_GENDER"





}
