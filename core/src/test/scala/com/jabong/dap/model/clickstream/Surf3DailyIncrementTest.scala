...
class Surf3DailyIncrementTest extends FlatSpec with SharedSparkContext {
  ...
  "DailyIncremetal for surf3" should "have 42 PDP records " in {
    var dailyIncrementalSKUs = VariableMethods.Surf3(userWiseData, userObj, hiveContext)
    dailyIncrementalSKUs.collect.foreach(println)
    assert(dailyIncrementalSKUs.collect.size == 42)
  }
