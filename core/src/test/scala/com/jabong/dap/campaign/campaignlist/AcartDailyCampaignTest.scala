...
class AcartDailyCampaignTest extends FeatureSpec with GivenWhenThen with SharedSparkContext {
  ...
  feature("Run acart daily campaigns") {
    scenario("Customer has abondoned the cart yesterday") {
      ...
      assert(acartPushCampaignOut._3 == "push_campaigns" && acartPushCampaignOut._2 == "acart_daily")
      assert(acartPushCampaignOut._1.count() == 1)
      assert(acartEmailCamapignOut._3 == "email_campaigns" && acartEmailCamapignOut._2 == "acart_daily")
    }
  }
