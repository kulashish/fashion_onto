...
/**
  ...
  */
class AcartIODCampaignTest extends FeatureSpec with GivenWhenThen with SharedSparkContext {
   ...
   feature("Run acart item on discount campaigns") {
     scenario("Customer has abondoned the cart in the last 30 days and the ref sku has less price now comapred to the day customer has added in cart") {
       ...
       assert(acartEmailCamapignOut._3 == "email_campaigns" && acartEmailCamapignOut._2 == "acart_iod")
     }
   }
