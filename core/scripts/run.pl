...
# Data Feeds
    ...
    $job_exit = run_component($component, $command);
     ...
     $job_exit = run_component($component, $command);
      ...
      my $command = "$BASE_SPARK_SUBMIT $AMMUNITION $HIVE_JARS $CORE_JAR --component loveCalendarCampaigns --config $HDFS_CONF/config.json --paramJson $HDFS_CONF/loveCalendarCampaigns.json";
      $job_exit = run_component($component, $command);
  }else {
