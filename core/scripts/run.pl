...
# erpAcq & merge
    ...
    my $command = "$BASE_SPARK_SUBMIT $AMMUNITION $HIVE_JARS $CORE_JAR --component basicItr --config $HDFS_CONF/config.json --varJson $HDFS_CONF/basicITR.json";
    run_component($component, $command);
    ...
    $AMMUNITION = "--num-executors 7 --executor-memory 9G";
     ...
     my $command = "$BASE_SPARK_SUBMIT $AMMUNITION $HIVE_JARS $CORE_JAR --component mobilePushCampaignQuality --config $HDFS_CONF/config.json --pushCampaignsJson $HDFS_CONF/pushCampaigns.json";
     run_component($component, $command);
 } else {
