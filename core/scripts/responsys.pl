...
system("hadoop fs -copyFromLocal 53699_SMS_OPT_OUT_$date_with_zero" . "_*.txt /data/input/responsys/sms_opt_out/daily/$date/.");

system("hadoop fs -mkdir -p /data/input/responsys/sms_delivered/daily/$date/");
system("hadoop fs -copyFromLocal 53699_SMS_DELIVERED_$date_with_zero" . "_*.txt /data/input/responsys/sms_delivered/daily/$date/.");
